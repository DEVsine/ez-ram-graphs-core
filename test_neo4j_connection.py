...
def test_neo4j_connection():
    ...
    try:
        ...
        # Test query
        results, meta = db.cypher_query("RETURN 'Hello from Neomodel!' as message, datetime() as timestamp")
        if results:
...
def test_with_sample_data():
    ...
    try:
        ...
        with driver.session() as session:
            ...
            # Query the test node
            result = session.run("MATCH (p:TestPerson) RETURN p.name as name, p.created_at as created_at")
            for record in result:
...
if __name__ == "__main__":
    ...
    print("2. Verify your credentials in project_name/settings.py")
    print("3. For Neo4j Desktop: default password is often 'neo4j' or what you set during setup")
